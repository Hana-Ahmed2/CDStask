...
interface BusinessUnitEditDialogProps {
  ...
  darkMode: boolean;
  onClose: () => void;
  onSave: (updatedUnit: BusinessUnit) => void;
}

export default function BusinessUnitEditDialog({
  open,
  businessUnit,
  darkMode,
  onClose,
  ...
  const validateForm = () => {
    ...
    setErrors(newErrors);
    return !newErrors.name && !newErrors.code;
  };

  const handleSave = () => {
    if (!validateForm() || !businessUnit) return;
